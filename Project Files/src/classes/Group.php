...
class Group
{
    ...
    {
        $sql = "SELECT gname, password, admin_id
                FROM groups
                WHERE id = :id;";
        return $this->db->runSQL($sql, [$id])->fetch();
    }

    public function create(array $group, string $destination): bool
    {
        $group['password'] = password_hash(($group['password']), PASSWORD_DEFAULT);

        try {
            $sql = "INSERT INTO groups (gname, password, admin_id)
                    VALUES (:gname, :password, :admin_id);";
            $this->db->runSql($sql, $group);
            return true;
        }
