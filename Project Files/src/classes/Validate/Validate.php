...
class Validate
{
    ...
    public static function isPhoneNum($phone_num)
    {

        $unwanted_chars = "/[-()]/";

        $phone_num = preg_replace($unwanted_chars, '', $phone_num );

        if ( mb_strlen($phone_num) == 11 and preg_match('^[0-9]+$', $phone_num) )
        {
            return true;
        }
        return false;
